...
double *simulate(const int i_max, const int t_max, double *old_array,
    ...
    if (process_Rank == 0) {
        for (t = 1; t < size_Of_Cluster - 1; t++) {
            // initialize (cur from [1 to n_local])
            printf("initialize cur from [1 to n_local] for process %d \n", t);
            for (int i = 1; i < n_local; i++) {
                cur[i] = current_array[i];
            }

            // send (t, cur[1:n_nlocal]
            printf("send (t, cur[1:n_nlocal] for process %d \n", t);
    ...
    printf("Looping over t-max for process %d \n", process_Rank);

    // loop over t-max
    for (t = 1; t < t_max; t++) {
        printf("Sending cur[1] to left_neightbor %d", left_neightbor);
        if (left_neightbor == -1) {
            MPI_Send(&cur[1], 1, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);
        }

        printf("Sending cur[n_local] to right_neightbor %d", right_neightbor);
        // cur[n_local + 1] = receive (right_neighbor)
        MPI_Recv(&cur[n_local + 1], 1, MPI_DOUBLE, right_neightbor, tag, MPI_COMM_WORLD, &status);

        printf("Sending cur[n_local] to right_neightbor %d", right_neightbor);
        // send(right_neighbor, cur[n_local])
        MPI_Send(&cur[n_local], 1, MPI_DOUBLE, right_neightbor, 0, MPI_COMM_WORLD);
