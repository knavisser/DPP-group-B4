...
double *simulate(const int i_max, const int t_max, double *old_array,
    ...
    if (process_Rank == 0) {
        cur[0] = 0;
        old[0] = 0;
    }else if (process_Rank == size_Of_Cluster - 1) {
        cur[n_local + 1] = 0;
        old[n_local + 1] = 0;
    }
    ...
    if (process_Rank > 0){
        printf("Process %d is send to process 0 (master)\n", process_Rank);
        fflush(0);
        for (int i = 1; i < n_local + 1; i++) {
            MPI_Send(&cur[i], 1, MPI_DOUBLE, 0, process_Rank, MPI_COMM_WORLD);
        }
    }
    ...
    if (process_Rank == 0) {
        ...
        for (int i = 1; i < size_Of_Cluster - 1; i++) {
            printf("Process 0 is receiving from process %d\n", i);
